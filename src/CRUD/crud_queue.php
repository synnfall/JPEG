...
function create_queue($conn, $userID, $ID_Jeux) {
    ...
    $sql = "INSERT INTO `queue`(`userID`, `gameID`,`token`, `date`) VALUES (?, ?, ?, NOW(3))";
    $stmt = mysqli_prepare($conn, $sql);
...
function update_queue($conn, $token) {
    $sql = "UPDATE `queue` SET `date` = NOW(3) WHERE `token` = ?";
...
function clean_queue($conn) {
    $sql = "DELETE FROM `queue` WHERE `date` < (NOW(3) - INTERVAL 5 SECOND)";
    $stmt = mysqli_prepare($conn, $sql);
