...
function get_lst_classement_par_jeux($conn, $nom_jeux) {
    $sql = "SELECT identifiant, pts FROM Classement JOIN Utilisateurs ON ID_User = UserID JOIN Jeux ON ID_Jeux = Jeux.ID WHERE nomJeux = ? GROUP BY ID_User ORDER BY pts DESC";
    $stmt = mysqli_prepare($conn, $sql);
    if (!$stmt) {
        echo "". mysqli_error($conn);
        return false;   
